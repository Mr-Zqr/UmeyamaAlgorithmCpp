#ifndef LIUMAYAMAALIGNMENT_H_
#define LIUMAYAMAALIGNMENT_H_
...
class UmeyamaAlignment
{
    private:
        ...
        static double calc_sigma(std::vector<Eigen::Vector3d> & x, Eigen::Vector3d miu_x, int point_size);
    public:
    UmeyamaAlignment();
    ~UmeyamaAlignment();
    static int align(std::vector<Eigen::Vector3d> & point_set_x, std::vector<Eigen::Vector3d> & point_set_y, Eigen::Matrix3d & R, Eigen::Vector3d & t, double & c);
};
